...
def get_farthest_pose_ids(tar_pose, ref_poses, num_select, tar_id=-1,angular_dist_method='vector', scene_center=(0, 0, 0)):
    ...
    selected_ids = []
    idx_ref = np.arange(num_cams)
    batched_pose_compare = batched_tar_pose
    for i in range(num_select):
        if angular_dist_method == 'matrix':
            dists = batched_angular_dist_rot_matrix(batched_pose_compare, ref_poses[:, :3, :3])
        elif angular_dist_method == 'vector':
            tar_cam_locs = batched_pose_compare[:, :3, 3]
            ref_cam_locs = ref_poses[:, :3, 3]
            scene_center = np.array(scene_center)[None, ...]
            tar_vectors = tar_cam_locs - scene_center
            ref_vectors = ref_cam_locs - scene_center
        ...
        selected_ids.append(idx_ref[selected_id])
        
        num_cams = num_cams - 1
        
        pose_compare = ref_poses[selected_id]
        batched_pose_compare = pose_compare[None].repeat(num_cams, 0)
        idx_ref = np.delete(idx_ref, selected_id)
        ref_poses = np.delete(ref_poses, selected_id, axis=0)

    return selected_ids

"""
def get_sparse_pose_ids(ref_poses, num_select,):
    num_cams = len(ref_poses)
    num_select = min(num_select, num_cams-1)

    ref_cam_locs = ref_poses[:, :3, 3]
    kmeans = KMeans(n_clusters=num_select, random_state=0).fit(ref_cam_locs)
    labels = kmeans.labels_

    ids_ref = np.arange(num_cams)
    selected_ids = []
    for i in range(num_select):
        ids_i = ids_ref[labels==i]
        ids_i_selected = np.random.choice(ids_i, 1)
        selected_ids.append(ids_i_selected)

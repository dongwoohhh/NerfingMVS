...
class DepthModelMeta(torch.nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, images, vars, metadata=None):
        """
        Images should be feed in the format (N, C, H, W). Channels are in BGR
        order and values in [0, 1].
        ...
        metadata["scales"]: (optional, can be None) specifies a post-scale
            transformation of the depth values. Format (1, N, 1).
        """
        depth = self.estimate_depth(images, vars)
    ...
    @abstractmethod
    def estimate_depth(self, images, vars) -> torch.Tensor:
        pass

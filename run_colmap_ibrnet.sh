...
for name in ${datadir}/* ; do
    ...
    python resize_image.py --datadir=${name} --factor $factor
    python gen_image_ids.py --datadir=${name}
    #rm -rf ${name}/images_1
    sh colmap.sh ${name}
    #python run_depth_priors.py --config configs/per_scene.txt --datadir=${name} --no_ndc --spherify --lindisp --expname=${name##*/} --factor
